    ...
    var LF = (function() {
        var self = {
            ...
            initParams: function() {
                ...
                if (typeof(LF.config.article_id) === 'undefined' && (typeof(LF.config.domain) === 'undefined' || LF.config.domain != 'thenextweb.fyre.co')) {
                    console.log("Livefyre Initialization: No article_id existed, so we're using the url instead.");
             ...
             */
            init: function() {
                var path = LF.servers.web +'/api/v1.1/internal/login_init.js';
            ...
            */
            ...
            loadCustomConfig: function() {
                ...
                } else if (document.cookie.indexOf('lf_config') > -1) {
                    config = LF.utils.cookie.get('lf_config');
        ...
        LF.login = function(profile) {
            if (typeof(profile.token) !== 'string') {
                console.log('Token must be provided as a string.');
                return LF;
            }
        ...
        LF.initCallback = function(response) {
            console.log('LF init callback:', response);
...
LF.Ajax = (function() {
     ...
     */
    self.XDR = $jl.XDR = function(url, args, callback, error_callback, http_method, transport) {
        ...
        if (typeof(error_callback) != 'function') {
    	    error_callback = function(r, status, thrownError) { 
        	    LF.Logger.log_default('JSONP Error', r, status, thrownError);
            };
        }
        ...
        if (typeof(args.lfgtoken) === 'undefined') {
            if (typeof(LF.data.user) !== 'undefined' && typeof(LF.data.user.token) !== 'undefined' && LF.data.user.token != '') {
        ...
        var callback_middleware = function(response, status, callback) {
            if (typeof(response) === 'string') {
                try {
                    ...
                    console.log("Error parsing result, but don't fail. Response = " + response);
                }
            }
            callback(response, status);
        };

        //LF.Logger.log_default('livefyretransport is', livefyre_transport);
        switch (transport) {
            case 'ajax': 
            ...
            case 'CORS':
                ...
                var statuscallback = function() {
                    //console.log('inside status callback');
                ...
                //request.onreadystatechange = function(o) {console.log('response was', o); }
            ...
            default:
                var areq = activeRequest = LF.Ajax.ajax({
                    ...
                    data: data,
                    success: function(data) {
                        callback_middleware(data, 'success', req.success);
                    },
                    dataType: 'jsonp'
                });
                return areq;
...
LF.Logger = (function() {
	...
	var test = function() {
		var logger = this;
		var original_logs = logs.main;
		var tests = []; // Array of functions
		var results = [];
		tests.push(function() {
			var title = 'After calling log(msg), the logs array size increases by 1';
			var before = logs.main.length;
			//console.log('Before: ', before);
			logger.log('Test something');
			var after = logs.main.length
			//console.log('After: ', after);
			return {title: title, result: (after == before+1)};
		});
		tests.push(function() {
			return {title: 'Dummy', result: true};
		});
	
		for (var i=0; i<tests.length; i++) {
			logger.clear();
			var test = tests[i]();
			console.log(test.title+": ", test.result);
			//test.func = tests[i];
			if (test.result==false) results.push(test);
		}
		if (results.length==0) {
			var message = 'No failures!';
		} else if (results.length>0) {
			var message = results.length + ' failures!';
		} else {
			var message = 'Uh oh';
		}
		console.log(message);
	
		logs.main = original_logs;
		return results;
	}
	var clear = function() {
		logs.main = [];
	...
	var print = function(o) {
		if (o=='v') {
			return $jl.JSON.stringify(logs);
		} else {
			return $jl.JSON.stringify(logs.main.map(function(e){
				return e.content;
			}));
		}
	};
	var log_default = function() {
		//use the default console.log and also send it to the livefyre log
		var msg="";
		for(var i=0; i<arguments.length; i++) 
	      msg = msg + " " + arguments[i]
		console.log(msg);
	...
	var log = function(level, msg, o) {
		...
		if (typeof(o)!=='undefined') {
			for (key in o) {
				if (o.hasOwnProperty(key)) record[key] = o[key];
			}
		};
		this.log_default(record.level+": "+record.content);
		logs['main'].push(record);
		return this;
	};
	var debug = function(event, msg, o) {
	    this.log('DEBUG', event +': '+ msg, o);
	};
	var info = function(event, msg, o) {
	    this.log('INFO', event +': '+ msg, o);
	};
	var error = function(event, msg, o) {
	    this.log('ERROR', msg, o);
	    this.send_solo('error', event, msg +' | '+ $jl.JSON.stringify(o));
	};
	var stream = function(msg, o) {
	    if ($jl('#lf_stream_console').length > 0 && $jl('#lf_stream_console').attr('on') == 'true') {
	        $jl('#lf_stream_console > .lf_logs').prepend('<b>'+ new Date() +'</b> : <b>'+ msg +'</b> : '+ $jl.JSON.stringify(o) +'<br><br>');
        }
	};
	...
	var length = function() {
		return logs.main.length;
	};
	var send = function() {
		if (!LF.detectMobile()) {
		    if (logs.main.length==0) {
    			console.log('LIVEFYRE.logger: No use sending an empty log');
    			return this;
    		}
    		this.log('LIVEFYRE.logger.send()');
    		console.log('Sending logs');
    		var current_length = logs.main.length;
    		var record = {
    			date: new Date(),
    			length: current_length,
    			payload: logs.main.slice(0,current_length)
    		}
    		/*
    		logs.meta['page_session'] = $jlPolling.page_session;
    		if (typeof(lfSocket) != 'undefined') {
    			logs.meta['lfSocket.transport'] = lfSocket.transport ? 'True' : 'False';
    			logs.meta['lfSocket.connected'] = lfSocket.connected || 'False';
    		}
    		logs.meta['$jl.pollCodeReverted'] = $jl.pollCodeReverted || 'False';
    		*/
    		logs.history.push(record);
    		var to_send = {
    			log: logs.main,
    			meta: logs.meta
    		}
    		if (LF.Ajax.ajax.ready()) {
        		LF.Ajax.ajax({
        			async: true,
        			type: "GET",
        			...
        			data: {
        				__LFLEVEL: 'info',
        				__LFEVENT: 'conv_id: '+ LF.data.conv.id,
        				__LFMSG: $jl.JSON.stringify(to_send),
        				__LFSITEID: LF.data.blog_id,
        				__LFSESSID: LF.Stream.page_session
        			},
        			success: function() {}
        		});
            }
	...
	var send_solo = function(level, event, message) {
	    if (!LF.detectMobile() && LF.Ajax.ajax.ready()) {
	        LF.Ajax.ajax({
    			...
    			data: {
    				...
    				__LFMSG: message,
    				__LFSITEID: LF.data.blog_id,
    				__LFSESSID: LF.data.conv ? LF.data.conv.id : null
    			},
    			success: function() {}
    		});
	    }
...
//if (typeof($jl)!='undefined') $jl('.assistly-widget a').live('click', function() { LIVEFYRE.logger.log("Clicking Assistly 'comment help?' link").send(); });

//console.log('Initializing LIVEFYRE.logger');
